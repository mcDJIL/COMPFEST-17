    ...
    static showToast(
        ...
        const toastHTML = `
            <div id="toast-${toastId}" class="fixed ${positionClass} z-50 toast-item">
                <div class="flex items-center w-full max-w-xs p-4 ${config.bgColor} rounded-lg shadow" role="alert">
                    ...
                    <div class="ml-3 text-sm font-normal">
                        <div class="text-sm font-semibold ${config.titleColor}">${toastTitle}</div>
                        <div class="text-sm ${config.textColor}">${message}</div>
                    </div>
                    <button type="button" class="ml-auto -mx-1.5 -my-1.5 ${config.bgColor} text-gray-400 hover:text-gray-900 rounded-lg focus:ring-2 focus:ring-gray-300 p-1.5 hover:bg-gray-100 inline-flex items-center justify-center h-8 w-8" data-dismiss-target="#toast-${toastId}" aria-label="Close">
